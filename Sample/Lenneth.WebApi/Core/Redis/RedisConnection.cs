...
namespace Lenneth.WebApi.Core.Redis
{
    ...
    internal class RedisConnection : IRedisConnection
    {
        ...
        /// 构造函数
        /// </summary>
        /// <param name="host">redis服务器</param>
        /// <param name="port">redis端口号</param>
        /// <param name="password">redis访问密码</param>
        /// <param name="collection">默认集合</param>
        public RedisConnection(string host, int port, string password = null, string collection = "0")
        {
            InitConnection(host: host, port: port, password: password,collection:collection);
        }
