...
namespace Lenneth.Core.Framework.LiteDB
{
    public partial class LiteCollection<T>
    {
        ...
        {
            return EnsureIndex(field, null, unique);
        }
        ...
        public bool EnsureIndex<TK>(Expression<Func<T, TK>> property, bool unique = false)
        {
            return EnsureIndex(property, null, unique);
        }
        ...
        /// <param name="expression">Create a custom expression function to be indexed</param>
        /// <param name="unique">Create a unique keys index?</param>
        public bool EnsureIndex<TK>(Expression<Func<T, TK>> property, string expression, bool unique = false)
        {
            var field = _visitor.GetField(property);

            return EnsureIndex(field, expression ?? _visitor.GetPath(property), unique);
        }
