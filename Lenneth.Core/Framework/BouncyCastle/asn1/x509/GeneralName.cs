...
namespace Lenneth.Core.FrameWork.BouncyCastle.asn1.x509
{
        ...
        : Asn1Encodable, IAsn1Choice
    {
		...
		private byte[] toGeneralNameEncoding(
			string ip)
		{
			if (BouncyCastle.util.net.IPAddress.IsValidIPv6WithNetmask(ip) || BouncyCastle.util.net.IPAddress.IsValidIPv6(ip))
			{
			...
			else if (BouncyCastle.util.net.IPAddress.IsValidIPv4WithNetmask(ip) || BouncyCastle.util.net.IPAddress.IsValidIPv4(ip))
			{
				int slashIndex = ip.IndexOf('/');
