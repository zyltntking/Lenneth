...
namespace Lenneth.Core.Framework.QR.Base
{
    public class QrCodeGenerator : IDisposable
    {
        ...
        public QrCodeData CreateQrCode(string plainText, EccLevel eccLevel, bool forceUtf8 = false, bool utf8Bom = false)
        {
            ...
            var codedText = PlainTextToBinary(plainText, encoding, utf8Bom, forceUtf8);
            ...
            var eccInfo = _capacityEccTable.Single(x => x.Version == version && x.ErrorCorrectionLevel.Equals(eccLevel));
        ...
        {
            var version = _capacityTable.Where(
                x => x.Details.Count(
        ...
        private string PlainTextToBinary(string plainText, EncodingMode encMode, bool utf8Bom, bool forceUtf8)
        {
            ...
            if (encMode.Equals(EncodingMode.Byte))
                return PlainTextToBinaryByte(plainText, utf8Bom, forceUtf8);
            return string.Empty;
        }
        ...
        private string PlainTextToBinaryAlphanumeric(string plainText)
        {
            ...
            while (plainText.Length >= 2)
            {
                ...
                var dec = _alphanumEncDict[token[0]] * 45 + _alphanumEncDict[token[1]];
                codeText += DecToBin(dec, 11);
                plainText = plainText.Substring(2);
            }
            if (plainText.Length > 0)
            {
                codeText += DecToBin(_alphanumEncDict[plainText[0]], 6);
            }
            return codeText;
        }
        ...
        {
            return _galoisField.Where(alog => alog.ExponentAlpha == exp).Select(alog => alog.IntegerValue).First();
        }

        private int GetAlphaExpFromIntVal(int intVal)
        {
            return _galoisField.Where(alog => alog.IntegerValue == intVal).Select(alog => alog.ExponentAlpha).First();
        }
        ...
        {
            _alphanumEncDict = new Dictionary<char, int>();
            ...
            {
                _alphanumEncDict.Add(res.Chr, res.Index);
            }
        }

        private void CreateAlignmentPatternTable()
        {
            _alignmentPatternTable = new List<AlignmentPattern>();
            ...
            for (var i = 0; i < (7 * 40); i = i + 7)
            {
                ...
                _alignmentPatternTable.Add(new AlignmentPattern()
                {
                    Version = (i + 7) / 7,
        ...
        {
            _capacityEccTable = new List<EccInfo>();
            for (var i = 0; i < (4 * 6 * 40); i = i + (4 * 6))
            {
                _capacityEccTable.AddRange(
                new[]
                {
                    new EccInfo(
                        (i+24) / 24,
        ...
        {
            _capacityTable = new List<VersionInfo>();
            for (var i = 0; i < (16 * 40); i = i + 16)
            {
                _capacityTable.Add(new VersionInfo(
        ...
        private void CreateAntilogTable()
        {
            ...
            for (var i = 0; i < 256; i++)
            {
                ...
                if (i > 7)
                {
                    gfItem = _galoisField[i - 1].IntegerValue * 2;
                }
                if (gfItem > 255)
                {
                    gfItem = gfItem ^ 285;
                }
                _galoisField.Add(new Antilog(i, gfItem));
            }
        }
        ...
        public void Dispose()
        {
            _alignmentPatternBaseValues = null;
            _alignmentPatternTable = null;
            _alphanumEncDict = null;
            _capacityBaseValues = null;
            _capacityEccBaseValues = null;
            _capacityEccTable = null;
            _capacityTable = null;
            _galoisField = null;
            _remainderBits = null;
        }
