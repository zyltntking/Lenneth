    ...
    internal static class PayloadGenerator
    {
            ...
            public override string ToString()
            {
                switch (_encoding)
                {
                    case MailEncoding.Mailto:
                        return
                            $"mailto:{_mailReceiver}?subject={Uri.EscapeDataString(_subject)}&body={Uri.EscapeDataString(_message)}";
                    case MailEncoding.Matmsg:
                        return
                            $"MATMSG:TO:{_mailReceiver};SUB:{EscapeInput(_subject)};BODY:{EscapeInput(_message)};;";
                    case MailEncoding.Smtp:
                        return
                            $"SMTP:{_mailReceiver}:{EscapeInput(_subject, true)}:{EscapeInput(_message, true)}";
                    default:
                        return _mailReceiver;
                }
            }
            ...
            public override string ToString()
            {
                switch (_encoding)
                {
                    case SmsEncoding.Sms:
                        return $"sms:{_number}?body={System.Uri.EscapeDataString(_subject)}";
            ...
            public override string ToString()
            {
                switch (_encoding)
                {
                    case MMSEncoding.MMSTO:
                        return $"mmsto:{_number}?subject={System.Uri.EscapeDataString(_subject)}";
                    case MMSEncoding.MMS:
                        return $"mms:{_number}?body={System.Uri.EscapeDataString(_subject)}";
                    default:
                        return "mms:";
                }
            }
        ...
        internal class CalendarEvent
        {
            ...
            public override string ToString()
            {
                ...
                if (encoding.Equals(EventEncoding.iCalComplete))
